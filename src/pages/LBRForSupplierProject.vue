<template>
  <q-page padding>
    ...
    <q-card class="q-pa-md">
      <div class="text-h6 q-mb-lg">List of Budget Requests for Supplier / Project</div>
...
defineOptions({
  name: 'LBRForSupplierProject'
})

const search = ref('')

// Updated pagination with sorting properties
  ...
  if (sortBy) {
    ...
    data.sort((a, b) => {
      ...
      if (sortBy === 'dateCreated') {
        ...
        const valA = column.field(a)?.toString().toLowerCase() ?? ''
        const valB = column.field(b)?.toString().toLowerCase() ?? ''
